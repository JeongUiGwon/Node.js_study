...
// POST 요청 시 컨텐트 타입이 application/x-www-form-rulencoded인 경우 파싱
...
app.post("/posts", (req, res) => {
  // 6. /posts로 요청이 오면 실행
  const { title, name, text } = req.body; // 7. HTTP로 요청의 body 데이터를 변수에 할당
...
app.delete("/posts/:id", (req, res) => {
  const id = req.params.id; // 9. app.delete에 설정한 path 정보에서 id 값을 가져옴
